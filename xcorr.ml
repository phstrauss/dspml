(* copy/paste from ocaml-fftw binding examples *)
...
let xcorr ?maxlag ?scale (a:D.vec) (b:D.vec) =
  ...
  let xcorr =
    if a == b then begin
      ...
      let fft = FFT.Array1.r2c ~meas:FFT.Estimate x y
      and ifft = FFT.Array1.c2r ~meas:FFT.Estimate y x in
      copy0 a x;
      FFT.exec fft;
      ...
      if dima < 10 * dimb && dimb < 10 * dima then (
        ...
        let ffta = FFT.Array1.r2c ~meas:FFT.Estimate x a'
        and fftb = FFT.Array1.r2c ~meas:FFT.Estimate x b' in
        let ifft = FFT.Array1.c2r ~meas:FFT.Estimate a' x in
        ...
        let x = create FFT.float n
        and b' = create FFT.complex n_complex in
        let fftb = FFT.Array1.r2c ~meas:FFT.Estimate x b' in
        copy0 b x;  FFT.exec fftb;
        for i = 1 to n_complex do
        ...
        let fft_xy = FFT.Array1.r2c ~meas:FFT.Estimate x y
        and fft_yx = FFT.Array1.c2r ~meas:FFT.Estimate y x in
        let i = ref 1 in
        while !i <= dima do
          ...
          FFT.exec fft_yx;
          (* D.axpy ~n:(l' + dimb - 1) c ~ofsy:(shift_seg !i) ~incy:inc_seg ; *)
          D.axpy ~n:(l' + dimb - 1) c ~ofsy:(shift_seg !i) ~incy:inc_seg x ;
          i := !i + l;
        done;
